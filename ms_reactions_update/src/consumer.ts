...
// consumer for the queue.reactionsUpdate
...
const consumer = (channel: Channel) => async (msg: ConsumeMessage | null): Promise<void> => {
  try {
    if (msg) {
      ...
      await banco.query(`UPDATE public."Reaction" SET reaction='${noticia.reaction}' WHERE "userId"=${userId} and"newId"=${newsId};`);
      channel.ack(msg);
    }
    else{
      console.log("vazio")
...
// Start the consumer
await channel.consume('reactionsUpdate', consumer(channel));